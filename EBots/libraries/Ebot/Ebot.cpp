...
void connectBluetoothTo(String strName, uint8_t type) {
    ...
    if (received.indexOf(strName) != -1) { //Check if the entered bluetooth device is found or not
        ...
        if (type == EBOT8PRO || type == EBOT4) {
            ...
            if (BtMac.substring(0, 1) == ":" && BtMac.substring(13, 15) == "OK") { //Verifying
    ...
    while (validData == 0) {
        ...
        if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
            ...
            switch (choice) {
                     ...
                     */
                case realAccelerationX: MPUdata = aaReal.x;
                    break;
                case realAccelerationY: MPUdata = aaReal.y;
                    break;
                case realAccelerationZ: MPUdata = aaReal.z;
                    break;

                case accelerationX: MPUdata = aaWorld.x;
                    break;
                case accelerationY: MPUdata = aaWorld.y;
                    break;
                case accelerationZ: MPUdata = aaWorld.z;
                    break;
                ...
                case angleZ: MPUdata = relativeAngle(ypr[0] * 180 / M_PI, offset);
...
void ebot_setup() {
	...
	if(!digitalRead(18) && !digitalRead(17) && !digitalRead(16)){ // if bridge mode is selected
	    bridge_setup();
		while(1){
			bridge_loop();
	 }
