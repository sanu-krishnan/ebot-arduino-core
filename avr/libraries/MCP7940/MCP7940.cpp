...
{                          // User compiler time to see RTC    //
  yOff = conv2d(date + 9); // Compute the year offset          //
  switch (date[0])
  { // Switch for month detection       //
  case 'J':
    m = (date[1] == 'a') ? 1 : ((date[2] == 'n') ? 6 : 7);
    break; // Jan Feb Mar Apr May Jun Jul Aug  //
  case 'F':
    m = 2;
    break; // Sep Oct Nov Dec                  //
  case 'A':
    m = date[2] == 'r' ? 4 : 8;
    break; //                                  //
  case 'M':
...
{                                                // Get the Date when power went off //
...
{                                                // Get the Date when power came back//
  uint8_t min, hr, day, mon;                     // temporary storage                //
  Wire.beginTransmission(MCP7940_ADDRESS);       // Address the I2C device           //
...
{                     // Write the DOW to the RTC         //
  uint8_t retval = 0; // assume we have an error          //
  if (dow > 0 && dow < 8)
  {                                   // If parameter is in range, then   //
...
{                        // Calibrate the RTC                //
  ...
  if (trim < 0)
    trim = B10000000 | (trim * -1);                                  // Set non-excess 128 negative val  //
...
{                                                          // Calibrate the RTC                //
  ...
  if (ppm > 130)
    ...
    ppm = -130;                            // Force number ppm to be in range  //
  ...
  if (trim < 0)
    osctrim = B10000000 | (trim * -1);                               // Set non-excess 128 negative val  //
  writeByte(MCP7940_CONTROL, readByte(MCP7940_CONTROL) & B11111011); // fine trim mode on, to be safe    //
...
{                                          // Get the trim register value      //
  int8_t trim = readByte(MCP7940_OSCTRIM); // read the register                //
  if (trim >> 7)
    trim = (B01111111 & trim) * -1; // if negative convert to excess128 //
...
*******************************************************************************************************************/
bool MCP7940_Class::getMFP()
{                                                                              // Get the MFP pin state            //
...
{                       //                                  //
  ...
  {                                                                      // if parameters and oscillator OK  //
    ...
    if (alarmNumber == 1)
      registerOffset = 7;                                                        // Otherwise use Alarm 1 registers  //
...
{ // Return alarm status              //
  ...
  if (alarmNumber == 1)
    registerNumber += 7;                                         // Otherwise use Alarm 1 registers  //
...
*******************************************************************************************************************/
bool MCP7940_Class::setSQWSpeed(uint8_t frequency, bool setState)
{                                                    // Set the SQW frequency code      //
...
{                                                                     // Enable or disable battery backup//
...
{                                                           // Return true on power fail state //
